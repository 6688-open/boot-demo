	...
	public long insertAndGetCurrentId(String tableName) {
		...
		if (tableName != null && !tableName.equals("")) {
			try {
				...
				throw new RuntimeException("SequenceManagerImpl--->获取表" + tableName + "的主键出现异常:" + e);
			}
		} else {
	...
	public List<String> insertAndGetIdLists(String tableName, int num) {
		...
		if (tableName != null && !tableName.equals("")) {
			if (num > maxNum) {
				...
				throw new RuntimeException("SequenceManagerImpl--->批量生成ID的数量不能大于" + maxNum);
			} else {
				try {
					for (int i = 0; i < num; i++) {
					...
					log.error("SequenceManagerImpl--->批量获取表" + tableName + "的主键出现异常:" + e);
					throw new RuntimeException("SequenceManagerImpl--->批量获取表" + tableName + "的主键出现异常:" + e);
				}
			}
		} else {
			log.error("SequenceManagerImpl--->表名不能为空值");
