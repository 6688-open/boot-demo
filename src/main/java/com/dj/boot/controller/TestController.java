...
public class TestController extends BaseController {
    ...
    public Response getReceiptsPerformType() {
        try {
            String parse = ReceiptsPerformTypeEnum.parse(1);
            return Response.success(ReceiptsPerformTypeEnum.OPTIONS);
        } catch (Exception e) {
    ...
    public Response complexAssemblyTest() {
        try {
     ...
     * 用对象 或者 String userName  都可以接到参数
     */
    @RequestMapping(value = "getShopByDeptId.do")
    public Object getShopByDeptId(Long deptId) throws Exception {

        if (null == deptId) {
            return null;
        }
        try {
            List<Shop> shopsList = Lists.newArrayList();
            if (deptId == 1L) {
                Shop shop1 = new Shop();
                shop1.setId(1L);
                shop1.setShopName("事业部B1-店铺1");
                Shop shop2 = new Shop();
                shop2.setId(2L);
                shop2.setShopName("事业部B1-店铺2");
                shopsList.add(shop1);
                shopsList.add(shop2);
            }
            ...
            if (deptId == 2L) {
                Shop shop1 = new Shop();
                shop1.setId(3L);
                shop1.setShopName("事业部B2-店铺1");
                Shop shop2 = new Shop();
                shop2.setId(4L);
                shop2.setShopName("事业部B2-店铺2");
                shopsList.add(shop1);
                shopsList.add(shop2);
            }
                if (deptId == 3L) {
                    Shop shop1 = new Shop();
                    shop1.setId(5L);
                    shop1.setShopName("事业部B3-店铺1");
                    Shop shop2 = new Shop();
                    shop2.setId(6L);
                    shop2.setShopName("事业部B3-店铺2");
                    shopsList.add(shop1);
                    shopsList.add(shop2);
                }
                return shopsList;
            ...
            log.error("查询店铺异常", e);
            throw new RuntimeException("查询店铺异常");
        }
    }



    @PostMapping(value = "translateUserRoleAndResource")
    public Response<Object> translateUserRoleAndResource(@RequestBody List<String> jsonList) {
