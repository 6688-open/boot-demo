 ...
 */
@Component
@Module(businessType = "RTW")
public class RtwBillExceptionServiceImpl implements ExceptionService {

    private static final Logger logger = LoggerFactory.getLogger(RtwBillExceptionServiceImpl.class);
    ...
    public Response retryException(BillExceptionRequest request) {
        logger.error("退货单处理入参:{}", JSONObject.toJSONString(request));
        try {
            BillExceptionDto billExceptionDto = (BillExceptionDto) request.getData();
            ...
            logger.error("RTW retryException  error", e);
