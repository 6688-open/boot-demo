...
/**
 * 代理类的调用处理程序，每个代理类的调用处理程序都必须实现InvocationHandler接口
 * 分发代理
 * @ProjectName: boot_demo
 * @PackageName: com.dj.boot.configuration.dispatch.proxy
 * @Date: 2020-07-17-10-41
 */
public class DispatchProxyHandler<T> implements InvocationHandler {
    private final Map<String, T> targetInstances;
    private final String defaultTarget;
    private final DispatchInvocationHandler handler;

    public DispatchProxyHandler(Map<String, T> targetInstances, String defaultTarget, DispatchInvocationHandler handler) {
     ...
     *
     *
     * 初始化时 实例化非懒加载 单例的bean时 遇到目标类被@Autowired时 属性注入 实例化bean时 创建代理类 交给IOC容器管理
     * 只要调用被代理类接口任何方法时 直接调用代理类的invoke方法来调用
     * 在自定义invoke里进行判断 调用哪个实例的方法
     * 代理类里面有被代理类信息  代理标识  在handler里找到对应的目标代理类 进行method.invoke(代理实例, args)
     */
    @Override
    ...
    public static <T> T newDispatchProxy(Class<T> targetInterface, Map<String, T> targetInstances, String defaultTarget,DispatchInvocationHandler handler) {
        ...
        DispatchProxyHandler<T> proxyHandler = new DispatchProxyHandler<T>(targetInstances,defaultTarget,handler);
