...
public class MapToBatch extends BootDemoApplicationTests {
    ...
    @Override
    public void run() {
        processBatchHandler(userService.list());
    }
     ...
     */
    @SneakyThrows
    private void processBatchHandler(List<User> list) {
        int len = list.size();
        // 计算size
        ...
        logger.error("==============================================================================================");
        for (int i = 0; i < size; i++) {
            int fromIndex = i * BATCH_NUM;
            int toIndex = (i == size - 1) ? len : (i + 1) * BATCH_NUM;
            List<User> userList = list.subList(fromIndex,toIndex);
            userList.forEach(System.out::println);
        }
