...
import com.dj.boot.common.functionInterface.VerifyParam;
...
//        }
        ...
        User user = new User().setId(1).setPassword("123456").setUserName("wj");
        if (StringUtils.isBlank(user.getUserName())) {
            response.setCode(BaseResponse.ERROR_PARAM);
            response.setMsg("参数缺失【userName】");
            logger.error(JSONObject.toJSONString(response));
        }
        if (StringUtils.isBlank(user.getPassword())) {
            response.setCode(BaseResponse.ERROR_BUSINESS);
            response.setMsg("参数缺失【password】");
            logger.error(JSONObject.toJSONString(response));
        }
        //用函数式接口替换
        User user1 = new User();
        verifyUser(BaseResponse.ERROR_PARAM, "参数缺失【userName】",response, (param1,param2) -> StringUtils.isBlank(user1.getUserName()));
        verifyUser(BaseResponse.ERROR_BUSINESS, "参数缺失【password】",response, (param1,param2) -> StringUtils.isBlank(user1.getPassword()));

    }

    private void verifyUser(Integer code, String msg, Response<Object> response, VerifyParam verifyParam) {
        if (verifyParam.verify("","")) {
    ...
    public  void testOne(FunctionInterfaceTest functionInterface, String age, Boolean isFail){
        if (functionInterface.verify(1, 2)) {
            LogUtils.info(age + isFail);
        }
    }
