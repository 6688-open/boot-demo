...
public class StreamApiTest extends BootDemoApplicationTests {
    ...
    public void run() {
        ...
        boolean bo = Iterators.any(list.listIterator(), new Predicate<User>() {
            @Override
            public boolean apply(@Nullable User user) {
                return StringUtils.equals(user.getUserName(), "1");
            }
        });
        ...
        //3 value 返回的是集合  取出第一条
        Map<Integer, User> lineNoMaps = Maps.uniqueIndex(list.iterator(), new Function<User, Integer>() {
            @Override
            public Integer apply(User user) {
                return user.getId();
            }
        });
        ImmutableListMultimap<Integer, User> flowMap = Multimaps.index(list, new Function<User, Integer>() {
            @Override
            public Integer apply(User user) {
                return user.getId();
            }
...
//=================================================================================================================================================================================
        ...
        String deptNos = Joiner.on(",").join(list5);
